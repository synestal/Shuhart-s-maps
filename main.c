/*Смысл программы - получаем от пользователя максимальное количество точек измерения в критериях (k).
Заполняем все k + rand() ячеек случайными числами, за исключением последних шести - они подчиняются третьему критерию.
